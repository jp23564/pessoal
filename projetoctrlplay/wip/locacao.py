...
class Locacao():
    try:
        ...
        def criar_locacao(cls):
            ...
            for locacao in lista_locacoes:
                if locacao.id_emprestimo == id_emprestimo:
        ...
        @classmethod
        def menu_locacoes(cls):
            escolha = input("O que deseja fazer?\n[1] Listar Locações\n[2] Criar Locação\n[3] Alterar Locação\n[4] Remover Locação\n[5] Salvar lista de Locações\n[6] Carregar lista de Locações\n")

            if escolha == '1':
                Locacao.listar_locacoes()
                Locacao.menu_locacoes()

            elif escolha == '2':
                Locacao.criar_locacao()
                Locacao.menu_locacoes()

            elif escolha == '3':
                Locacao.alterar_locacao()
                Locacao.menu_locacoes()
            
            elif escolha == '4':
                Locacao.remover_locacao()
                Locacao.menu_locacoes()

            elif escolha == '5':
                Locacao.salvar_lista()
                Locacao.menu_locacoes()

            elif escolha == '6':
                Locacao.carregar_locacoes()
                Locacao.menu_locacoes()
